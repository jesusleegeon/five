#include <stdio.h>
#define MAX_COUNT  5//등록 가능한 최대 수 
...
int AddFriend(NAME_TYPE*p_name, unsigned short int*p_age, float*p_height, float*p_weight, int count)
{
	if(count < MAX_COUNT)
	{
		...
		printf("입력을 완료했습니다. \n\n");
		return 1;   //친구 추가 성공을 하면 1을 반환함 
	} else
	{
		printf("최대 인원을 초과하여 입력을 할 수 없스니다. \n");
...
void ShowFriendList(NAME_TYPE, unsigned short int*p_age, float*p_height, float*p_weight, int count)
{
	int i;
